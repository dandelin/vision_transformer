...
def create_learning_rate_schedule(
  ...
  """
    ...
    def step_fn(step):
        ...
        if warmup_steps:
            lr = lr * np.minimum(1.0, step / warmup_steps)
...
def lr_prefetch_iter(
    lr_fn, first_step, total_steps, prefetch_to_device=2, devices=None
):
    local_device_count = jax.local_device_count() if devices is None else len(devices)
    lr_iter = (
        np.ones([local_device_count]) * lr_fn(i) for i in range(first_step, total_steps)
    )
