...
def get_dataset_info(dataset, split):
    ...
    return {"num_examples": num_examples, "num_classes": num_classes}
...
def get_data(
  ...
  """
    ...
    def _pp(data):
        ...
        im = (im - 127.5) / 127.5
        label = tf.one_hot(
            data["label"], dataset_info["num_classes"]
    ...
    def _mixup(data):
        ...
        data["image"] = beta * data["image"] + (1 - beta) * tf.reverse(
            data["image"], axis=[0]
        )
        data["label"] = beta * data["label"] + (1 - beta) * tf.reverse(
            data["label"], axis=[0]
        )
        return data
    ...
    def _shard(data):
        data["image"] = tf.reshape(
            data["image"], [num_devices, -1, crop_size, crop_size, 3]
        )
