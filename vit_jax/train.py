...
def main(args):
    ...
    lr_fn = hyper.create_learning_rate_schedule(
        total_steps, args.base_lr, args.decay_type, args.warmup_steps
    )
    ...
    for step, batch, lr_repl in zip(
        ...
        input_pipeline.prefetch(ds_train, args.prefetch),
        lr_iter,
    ):
        ...
        if args.progress_every and step % args.progress_every == 0:
            ...
            logger.info(
                f"Step: {step}/{total_steps} {100*done:.1f}%, "
        ...
        if (args.eval_every and step % args.eval_every == 0) or (step == total_steps):
            ...
            accuracy_test = np.mean(
                [
                    ...
                    for c in (
                        np.argmax(vit_fn_repl(opt_repl.target, batch["image"]), axis=2)
                        == np.argmax(batch["label"], axis=2)
                    ).ravel()
                ]
            )
